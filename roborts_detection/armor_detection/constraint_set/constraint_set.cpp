...
void ConstraintSet::LoadParam() {
  ...
  radio_ = constraint_set_config_.threshold().radio();
...
//          ROS_WARN("time sleep %lf", (capture_time - detection_time_));
  ...
  auto detection_begin = std::chrono::high_resolution_clock::now();
    ...
    if(!armors.empty()) {
      ...
      cv_toolbox_->DrawRotatedRect(final_, final_armor.rect, cv::Scalar(0, 255, 0), 2);
  ...
  for (unsigned int i = 0; i < contours_brightness.size(); ++i) {
    for (unsigned int j = 0; j < contours_light.size(); ++j) {
        if (cv::pointPolygonTest(contours_light[j], contours_brightness[i][0], false) >= 0.0) {
          ...
          /*
          if (light_info.angle_ > 95) {
            float temp = single_light.size.width;
            single_light.size.width = single_light.size.height;
            single_light.size.height = temp;
          }
          */
          single_light.angle = light_info.angle_;
          cv::Point2f vertices_[4] = {light_info.vertices_[0], light_info.vertices_[1], light_info.vertices_[2], light_info.vertices_[3]};
          cv::RotatedRect true_light = cv::RotatedRect(cv::Point2f(light_info.center_.x, light_info.center_.y), cv::Size2f(light_info.width_, light_info.height_), 
                                                      light_info.angle_);

          /*
          //single_light.angle = 0;
          /*
          ...
          /*
          //filter 
          auto light_aspect_ratio = std::max(single_light.size.width, single_light.size.height) / std::min(single_light.size.width, single_light.size.height);
          //single_light.angle = (single_light.angle > 90 ? single_light.angle - 180 : single_light.angle);

          if (enable_debug_) {
            std::cout << "light angle: " << single_light.angle << std::endl;
            std::cout << "light.size.width: " << single_light.size.width << std::endl;
            std::cout << "light.size.height: " << single_light.size.height << std::endl;
            std::cout << "light.center.x: " << single_light.center.x << std::endl;
            std::cout << "light.center.y: " << single_light.center.y << std::endl;
            std::cout << "light_aspect_ratio: " << light_aspect_ratio << std::endl;
            std::cout << "light_area: " << single_light.size.area() << std::endl;
          }
          
          if (light_aspect_ratio < light_max_aspect_ratio_ &&
            single_light.size.area() >= light_min_area_ && single_light.angle < light_max_angle_ &&
            light_aspect_ratio > light_min_aspect_ratio_ && single_light.size.height > single_light.size.width) { //angle < light_max_angle_ &&
              lights.push_back(single_light);
              if (enable_debug_)
                cv_toolbox_->DrawRotatedRect(show_lights_after_filter_, single_light, cv::Scalar(0, 255, 0), 2);
            }
          break;
          */
          if (true_light.angle > 60 && true_light.angle < 120)
            lights.push_back(true_light);
            if (enable_debug_)
              cv_toolbox_->DrawRotatedRect(show_lights_after_filter_, true_light, cv::Scalar(0, 255, 0), 2);
        }
    }
  }

  if (enable_debug_)
    cv::imshow("show_lights_after_filter", show_lights_after_filter_);
...
void ConstraintSet::FilterLights(std::vector<cv::RotatedRect> &lights) {
  ...
  for (const auto &light : lights) {
    ...
    if (light_aspect_ratio < light_max_aspect_ratio_ &&
        ...
        light_aspect_ratio > light_min_aspect_ratio_) { //angle < light_max_angle_ &&
  ...
  for (unsigned int i = 0; i < lights.size(); i++) {
    for (unsigned int j = i + 1; j < lights.size(); j++) {
      ...
      if (angle_diff < light_max_angle_diff_ &&
          std::max<float>(edge1.second, edge2.second)/std::min<float>(edge1.second, edge2.second) < radio_ &&
          ...
          gray_img_.at<uchar>(static_cast<int>(rect.center.y), static_cast<int>(rect.center.x))
              ...
              rect.size.width / (rect.size.height) > armor_min_aspect_ratio_) { //std::abs(center_angle) < armor_max_angle_ &&
...
*/
  std::sort(armors.begin(),
            armors.end(),
            [](const ArmorInfo &p1, const ArmorInfo &p2) { return (p1.rect.size.width / p1.rect.size.height) < (p2.rect.size.width / p2.rect.size.height); });
