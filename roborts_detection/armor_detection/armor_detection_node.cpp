...
ErrorInfo ArmorDetectionNode::Init() {
  ...
  ref_sub_ = enemy_nh_.subscribe<roborts_msgs::GameStatus>("/game_status", 1, &ArmorDetectionNode::RefereeCB, this);
...
void ArmorDetectionNode::RefereeCB(const roborts_msgs::GameStatus::ConstPtr &status) {
  status_ = *status;
  }
  ...
  while(running_) {
    ...
    if (node_state_ == NodeState::RUNNING) {
      ...
      if(detected_enemy_) {
        ...
        /*
        if (!init_) {
          target_3d.y = gimbal_control_.Filter(target_3d.y, init_);
          init_ = 1;
        }
        std::cout << "target_3d.y_before: " << target_3d.y << std::endl;
        target_3d.y = gimbal_control_.Filter(target_3d.y, init_);
        */
        gimbal_control_.Transform(target_3d, pitch, yaw);
        std::cout << "target_3d.y_after: " << target_3d.y << std::endl;
